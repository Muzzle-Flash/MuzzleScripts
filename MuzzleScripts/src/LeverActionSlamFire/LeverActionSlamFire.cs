...
{
	public class LeverActionSlamFire : BaseUnityPlugin
    {

		private void Awake()
        {
			Hook();
			base.Logger.LogDebug(string.Concat(new string[]
			{
				"LeverActionFirearm has loaded!"
			}));
        ...
        private void LeverActionFirearm_UpdateLever(On.FistVR.LeverActionFirearm.orig_UpdateLever orig, LeverActionFirearm self)
        {
			...
			bool flag4 = false;
			if ((self.m_isHammerCocked || self.m_isHammerCocked2) && !self.m_isSpinning && self.m_curLeverPos == FistVR.LeverActionFirearm.ZPos.Rear)
			{
				flag4 = true;
			}
			if (flag3)
			...
			{
				if (self.m_lastLeverPos == FistVR.LeverActionFirearm.ZPos.Forward)
				{
					self.m_curLeverPos = FistVR.LeverActionFirearm.ZPos.Middle;
				}
				else
				{
					self.m_curLeverPos = FistVR.LeverActionFirearm.ZPos.Rear;
					self.IsBreachOpenForGasOut = false;
				}
			}
			else if (Mathf.Abs(self.m_curLeverRot - self.LeverAngleRange.x) < 1f)
			{
				if (self.m_lastLeverPos == FistVR.LeverActionFirearm.ZPos.Rear)
				{
					self.m_curLeverPos = FistVR.LeverActionFirearm.ZPos.Middle;
				}
				else
				{
					self.m_curLeverPos = FistVR.LeverActionFirearm.ZPos.Forward;
			...
			if (self.m_curLeverPos == FistVR.LeverActionFirearm.ZPos.Rear && self.m_lastLeverPos != FistVR.LeverActionFirearm.ZPos.Rear)
			{
				...
				if (self.m_isActionMovingForward && self.m_proxy.IsFull && !self.Chamber.IsFull)
				{
					...
					if (self.m_hand.Input.TriggerPressed == true)
					{
						self.Fire();
					}
			...
			else if (self.m_curLeverPos == FistVR.LeverActionFirearm.ZPos.Forward && self.m_lastLeverPos != FistVR.LeverActionFirearm.ZPos.Forward)
			{
			...
			else if (self.m_curLeverPos == FistVR.LeverActionFirearm.ZPos.Middle && self.m_lastLeverPos == FistVR.LeverActionFirearm.ZPos.Rear)
			{
			...
			if (self.m_curLeverPos != FistVR.LeverActionFirearm.ZPos.Rear && !self.m_proxy.IsFull)
			{
			...
			if (self.UsesSecondChamber)
			{
				if (self.m_curLeverPos != FistVR.LeverActionFirearm.ZPos.Rear && !self.m_proxy2.IsFull)
				{
